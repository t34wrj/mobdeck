...
import { getSecurityHeaders, getEnhancedSecurityHeaders, validateUrl } from '../../src/utils/security';
...
describe('API Communication Security Tests', () => {
  ...
  describe('URL Validation Security', () => {
    ...
    it('should sanitize URLs with spaces and special characters', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.sanitized).toBe('https://example.com/path%20with%20spaces/%3Ctag%3E');
    });
    ...
    it('should enforce URL length constraints', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.error).toBe('URL length must be between 10 and 2048 characters');
      
      const longUrl = 'https://example.com/' + 'a'.repeat(2050);
      const longResult = validateUrl(longUrl);
      expect(longResult.isValid).toBe(false);
      expect(longResult.error).toBe('URL length must be between 10 and 2048 characters');
    });
  });
  ...
  describe('Cache Security', () => {
    it('should prevent caching of sensitive data', () => {
      const enhancedHeaders = getEnhancedSecurityHeaders();
      expect(enhancedHeaders['Cache-Control']).toBe('no-cache, no-store, must-revalidate');
  ...
  describe('Security Configuration Validation', () => {
    it('should use secure defaults for all headers', () => {
      ...
      expect(headers['Content-Security-Policy']).toContain("default-src 'self'"); // Restrictive CSP
      expect(headers['Strict-Transport-Security']).toContain('max-age=31536000'); // 1 year HSTS
      expect(headers['Referrer-Policy']).toBe('strict-origin-when-cross-origin'); // Secure referrer
    });
