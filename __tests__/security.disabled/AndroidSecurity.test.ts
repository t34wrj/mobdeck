...
describe('Android Security Configuration Tests', () => {
  ...
  describe('AndroidManifest.xml Security', () => {
    ...
    it('should only declare necessary permissions', () => {
      ...
      const declaredPermissions = Array.from(permissions).map(p => 
    ...
    it('should properly configure exported components', () => {
      ...
      const mainActivity = Array.from(activities).find(activity =>
      ...
      const shareActivity = Array.from(activities).find(activity =>
      ...
      const backgroundService = Array.from(services).find(service =>
      ...
      const bootReceiver = Array.from(receivers).find(receiver =>
  ...
  describe('Build Security Configuration', () => {
    ...
    it('should not contain hardcoded secrets', () => {
      ...
      const suspiciousKeys = nonCommentLines.match(/[A-Za-z0-9]{32,}/g) || [];
      const allowedKeys = [
  ...
  describe('Network Security Configuration', () => {
    ...
    it('should disable cleartext traffic by default', () => {
      const baseConfig = networkConfigDoc.getElementsByTagName('base-config')[0];
    ...
    it('should allow cleartext for localhost development', () => {
      ...
      Array.from(domainConfigs).forEach(config => {
      ...
      expect(localhostConfig?.getAttribute('cleartextTrafficPermitted')).toBe('true');
    });
    ...
    it('should trust system certificates', () => {
      ...
      Array.from(trustAnchors).forEach(anchor => {
