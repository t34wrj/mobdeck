...
class DatabaseService implements DatabaseServiceInterface {
                    ...
                    try {
                        const context: TransactionContext = {
                            executeSql: (sql: string, params?: any[]) => {
                                return new Promise<DatabaseResult>((resolveQuery, rejectQuery) => {
                                    tx.executeSql(
                                        sql,
                                        params || [],
    ...
    public async createArticle(
        ...
        try {
            ...
            const params = [
                ...
                article.url, article.image_url, article.read_time,
                article.is_archived, article.is_favorite, article.is_read,
                article.source_url, now, now, article.synced_at,
                article.is_modified, article.deleted_at
            ];
    ...
    public async getArticle(id: string): Promise<DatabaseOperationResult<DBArticle>> {
        try {
    ...
    public async getArticles(
        ...
        try {
            const { whereClause, params, countParams } = this.buildArticleQuery(filters);
    ...
    public async removeLabelFromArticle(
        ...
        try {
            const sql = 'DELETE FROM article_labels WHERE article_id = ? AND label_id = ?';
    ...
    public async getArticleLabels(articleId: string): Promise<DatabaseOperationResult<DBLabel[]>> {
        try {
            const sql = `
    ...
    public async getLabelArticles(labelId: number): Promise<DatabaseOperationResult<DBArticle[]>> {
        try {
            const sql = `
                SELECT a.* FROM articles a
    ...
    public async createSyncMetadata(
        ...
        try {
            ...
            const params = [
                metadata.entity_type, metadata.entity_id, metadata.operation,
                metadata.local_timestamp, metadata.server_timestamp,
                metadata.sync_status, metadata.conflict_resolution,
                metadata.retry_count, metadata.error_message, now, now
            ];
    ...
    public async deleteSyncMetadata(id: number): Promise<DatabaseOperationResult> {
        try {
    ...
    public async runMigrations(migrations: Migration[]): Promise<DatabaseOperationResult> {
        try {
            ...
            await this.executeInTransaction(async (ctx) => {
                for (const migration of pendingMigrations) {
                    console.log(`[DatabaseService] Running migration ${migration.version}: ${migration.description}`);
                    
                    // Execute migration in a nested transaction context
                    await migration.up({
                        executeSql: (sql, params, success, error) => {
                            ctx.executeSql(sql, params)
    ...
    // Helper Methods
    private async executeSql(sql: string, params: any[] = []): Promise<DatabaseResult> {
        if (!this.db) {
            throw this.createDatabaseError(
                DatabaseErrorCode.CONNECTION_FAILED,
                'Database not connected'
...
export const DatabaseUtilityFunctions: DatabaseUtils = {
    ...
    convertDBLabelToLabel(dbLabel: DBLabel): Label {
        return {
            ...
            syncedAt: dbLabel.synced_at ? new Date(dbLabel.synced_at * 1000) : undefined
        };
    },
    ...
    convertLabelToDBLabel(label: Label): DBLabel {
        return {
            ...
            synced_at: label.syncedAt ? Math.floor(label.syncedAt.getTime() / 1000) : null
        };
    },
