 ...
 */
export const errorHandlerMiddleware: Middleware<{}, RootState> = (store) => (next) => (action: AnyAction) => {
  try {
    return next(action);
  } catch (error) {
 ...
 */
export const performanceMiddleware: Middleware<{}, RootState> = (store) => (next) => (action: AnyAction) => {
  if (__DEV__) {
    ...
    if (endTime - startTime > 10) {
      console.warn(`⚠️ Slow action detected: ${action.type} took ${(endTime - startTime).toFixed(2)}ms`);
    }
