...
import { selectAllArticles, selectArticleById, selectArticleEntities } from '../slices/articlesSlice';
...
export const selectSyncStatus = createSelector(
  [selectIsSyncing, selectLastSyncTime, selectHasPendingChanges, selectHasSyncConflicts, selectSyncError],
  (isSyncing, lastSyncTime, hasPendingChanges, hasSyncConflicts, syncError) => ({
    isSyncing,
    lastSyncTime,
    hasPendingChanges,
    hasSyncConflicts,
    ...
    status: isSyncing ? 'syncing' : 
            syncError ? 'error' : 
            hasSyncConflicts ? 'conflicts' : 
            hasPendingChanges ? 'pending' : 'synced'
  })
);
...
export const selectSelectedArticles = createSelector(
  ...
  (selectedIds, articles) => selectedIds.map(id => articles[id]).filter(Boolean) as Article[]
);
...
export const selectFavoriteArticles = createSelector(
  [selectAllArticles],
  (articles) => articles.filter(article => article.isFavorite && !article.isArchived)
);
...
export const selectRecentArticles = createSelector(
  ...
  (articles) => {
    ...
    return articles.filter(article => article.createdAt > oneDayAgo && !article.isArchived);
  }
);
...
export const selectArticlesByIds = createSelector(
  ...
  (articles, articleIds) => articleIds.map(id => articles[id]).filter(Boolean) as Article[]
);
...
export const selectArticleConflictStatus = createSelector(
  [selectSyncConflicts, selectPendingChanges, (_: RootState, articleId: string) => articleId],
  (conflicts, pendingChanges, articleId) => ({
    ...
    needsSync: conflicts.includes(articleId) || pendingChanges.includes(articleId),
  })
);
  ...
  (articles, pendingChanges, conflicts) => {
    return articles.map(article => ({
      ...
      syncStatus: {
        ...
        needsSync: conflicts.includes(article.id) || pendingChanges.includes(article.id),
      }
    }));
  }
);
...
export const selectArticleNavigationInfo = createSelector(
  ...
  (articles, selectedId) => {
    ...
    return {
      ...
      nextArticleId: currentIndex < articles.length - 1 ? articles[currentIndex + 1].id : null,
      previousArticleId: currentIndex > 0 ? articles[currentIndex - 1].id : null,
    };
  }
);
