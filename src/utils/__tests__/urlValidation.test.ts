...
describe('URL Validation Utilities', () => {
  describe('validateUrl', () => {
    ...
    it('should validate URLs with query parameters', () => {
      const result = validateUrl('https://example.com/article?id=123&ref=share');
      
      expect(result.isValid).toBe(true);
      expect(result.normalizedUrl).toBe('https://example.com/article?id=123&ref=share');
      expect(result.errors).toHaveLength(0);
    });
    ...
    it('should reject URLs with invalid protocols', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Protocol \'ftp\' is not allowed. Allowed protocols: http, https');
    });
    ...
    it('should validate allowed domains', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Domain \'untrusted.com\' is not in the allowed domains list');
    });
    ...
    it('should warn about HTTP URLs', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain('Using HTTP instead of HTTPS may be insecure');
    });
    ...
    it('should warn about localhost URLs', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain('URL points to a local or private address');
    });
    ...
    it('should warn about URL shorteners', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain('URL appears to be shortened - consider expanding for security');
    });

    it('should warn about suspicious query parameters', () => {
      const result = validateUrl('https://example.com/article?javascript=alert(1)');
      
      expect(result.isValid).toBe(true);
      expect(result.warnings).toContain('URL contains potentially suspicious query parameters');
    });
  });
  ...
  describe('extractUrlFromText', () => {
    it('should extract HTTP URLs from text', () => {
      const text = 'Check out this article: http://example.com/article and let me know!';
    ...
    it('should extract HTTPS URLs from text', () => {
      const text = 'Check out this article: https://example.com/article and let me know!';
      const result = extractUrlFromText(text);
  ...
  describe('isLikelyArticleUrl', () => {
    it('should identify article URLs with article path', () => {
      const result = isLikelyArticleUrl('https://example.com/article/how-to-code');
      expect(result).toBe(true);
    });

    it('should identify blog post URLs', () => {
      const result = isLikelyArticleUrl('https://example.com/blog/my-first-post');
      expect(result).toBe(true);
    });

    it('should identify news URLs', () => {
      const result = isLikelyArticleUrl('https://news.example.com/story/breaking-news');
      expect(result).toBe(true);
    });

    it('should identify date-based URLs', () => {
      const result = isLikelyArticleUrl('https://example.com/2024/01/article-title');
      expect(result).toBe(true);
    });

    it('should identify URLs with multiple path segments', () => {
      const result = isLikelyArticleUrl('https://example.com/section/subsection/article-title');
      expect(result).toBe(true);
    });
