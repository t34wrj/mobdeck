...
export function validateUrl(
  ...
  if (
    trimmedUrl.length < INPUT_CONSTRAINTS.URL.minLength ||
    trimmedUrl.length > maxLength
  ) {
    return {
      isValid: false,
  ...
  return {
    isValid: true,
    value:
      urlValidationResult.normalizedUrl ||
...
export function validateSearchQuery(
  ...
  // Check length constraints
  if (
    trimmedQuery.length < opts.minLength ||
    trimmedQuery.length > opts.maxLength
  ) {
    return {
      isValid: false,
...
export function validateLabelName(
  ...
  // Check length constraints
  if (
    trimmedName.length < opts.minLength ||
    trimmedName.length > opts.maxLength
  ) {
    return {
      isValid: false,
...
export function validateArticleTitle(
  ...
  // Check length constraints
  if (
    trimmedTitle.length < opts.minLength ||
    trimmedTitle.length > opts.maxLength
  ) {
    return {
      isValid: false,
  ...
  if (hasXSSPatterns(trimmedTitle)) {
    return {
      isValid: false,
      error: 'Article title contains invalid characters',
    };
  }
  ...
  return validateFormData(formData, {
    syncInterval: {
      validator: (value: string | number) =>
        validateNumericInput(value, { type: 'syncInterval' }),
      required: false,
    },
    batchSize: {
      validator: (value: string | number) =>
        validateNumericInput(value, { type: 'batchSize' }),
      required: false,
    },
  });
}
